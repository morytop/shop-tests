...
`TEST_PLAN.md` is the living test plan: scope, out-of-scope items, data strategy, the tag taxonomy, and a full feature-area → spec-file mapping. Much of it is now implemented — `tests/ui/` holds `login`, `register`, `forgot-password`, `change-password`, `profile`, `totp-setup`, `cart`, `category`, `checkout-address`, `checkout-e2e`, `checkout-payment`, `checkout-signin`, `favorites`, `product-detail`, `product-filters`, `product-overview`, `product-search`, and `rentals` specs plus `smoke/{homepage,menu}`, `tests/admin/` holds the read-only `dashboard`/`sections` admin smoke sweep, and `tests/api/` holds the catalog-read/write-rejection specs (`products`, `brands`, `categories`, `images`, `catalog`), the `users` area (`users.smoke`, `users.register`, `users.login`, `users.session`, `users.account`, `totp.negative`), the `carts` area (`carts.lifecycle`, `carts.negative`), and the `favorites`, `invoices`, `messages`, `payment`, and `postcode` areas; check the per-section status notes in `TEST_PLAN.md` for what each area covers and what's still deferred. TOTP has no spec file of its own: enrolment lives in `totp-setup.spec.ts`, while logging in with a code is covered inside `login.spec.ts`. Check it before writing tests for a feature area, and update its coverage map when adding specs.
...
- **API layer** (`src/api/`): mirrors the UI layer — `requests/` (`BaseRequest` with the generic REST verbs `get`/`getOne`/`post`/`put`/`patch`/`delete`, extended by one request object per resource: `users`, `login`, `totp`, `products`, `brands`, `categories`, `carts`, `favorites`, `invoices`, `messages`, `payment`, `reports` — all hitting `API_URL`), `models/` (snake_case wire payloads: user/login/cart/favorite/invoice/message/payment), `factories/` (`prepareRandomUserPayload()`/`registerUserWithApi()` reuse the UI `prepareRandomUser()` as the single data source; `getAuthorizationHeader()` → Bearer token; `registerUserWithTotpEnabled()` registers a throwaway user and enrols it in TOTP over the API, for specs that need a 2FA account as a precondition; and the Phase G UI-arrange factories `addFavoritesWithApi()`/`createInvoiceWithApi()`/`sendMessageWithApi()`, which file per-user data over the API so a UI spec can skip an arrange whose flow another test already exercises through the UI — each feature area must keep at least one UI-driven basic scenario, and cart seeding via browser storage is explicitly rejected, see `PRODUCT_EXPLORATION.md` §8), `fixtures/request-object.fixture.ts`, `data/` (fixed values and the data-driven validation tables: `catalog.data.ts`, `invalid-user-payloads.ts`, `non-standard-inputs.ts` — kept pure, so a table takes the valid payload as an argument rather than importing a factory), and `utils/api.util.ts` (`API_PATHS` + the derived `apiUrls` map of absolute endpoint URLs, since the API host differs from the UI `baseURL` — every new endpoint gets added there). API factories are the one sanctioned place for `expect()` outside specs. `TotpRequest` is constructed with an auth header rather than injected as a fixture, so it is reached through the factory, not as a test arg.
- **Generated contract schemas** (`src/api/schemas/`): `toolshop.zod.ts` (one strict Zod schema per documented operation) and `spec.hash` are **generated — never hand-edit them**. Regenerate with `npm run generate:api-schemas`: it fetches the live OpenAPI docs, normalizes them (`scripts/normalize-spec.ts` — 3.2→3.1 patch, `query`-op strip, required-ification, since the published spec declares no `required`/`additionalProperties` at all), applies the known doc/behaviour deviations overlay (`scripts/schema-deviations.ts`, one entry per mismatch, each citing `PRODUCT_EXPLORATION.md` §6), and runs orval in Zod client mode. The `@contract` specs in `tests/api/contract/` assert response bodies via `expectToMatchSchema()` (`src/api/utils/schema.util.ts`); `contract.freshness.api.spec.ts` compares the live docs hash against the committed `spec.hash`, so docs drift fails loudly with the regeneration command. Triage a red contract test as a doc/behaviour mismatch: record it in `PRODUCT_EXPLORATION.md`, then it's either an API regression (leave it red) or a stale doc (add an overlay entry).
- **Request-object fixtures** (`request-object.fixture.ts`) come in three tiers: anonymous (`usersRequest`, `loginRequest`, `productsRequest`, `brandsRequest`, `categoriesRequest`, `imagesRequest`, `cartsRequest`, `favoritesRequest`, `invoicesRequest`, `messagesRequest`, `paymentRequest`); customer-authenticated `*RequestLogged` variants (`usersRequestLogged`/`favoritesRequestLogged`/`invoicesRequestLogged`, plus `productsRequestLogged`/`brandsRequestLogged`/`categoriesRequestLogged` which exist only to prove catalog writes are _rejected_ for a non-admin token) that register one fresh throwaway user per test (exposed as `loggedApiUser`) and share its Bearer token — only pull these in specs that need an authenticated customer, since each use writes a user to the shared backend; and admin `*RequestAdmin` variants (`usersRequestAdmin`/`messagesRequestAdmin`/`reportsRequestAdmin`) that log in as the seeded admin with the correct password only and are strictly for GETs.
- **`BaseRequest` sends `Accept: application/json` on every request, and must keep doing so.** The backend is Laravel: without that header it answers a validation failure with a **302 to the API root**, which Playwright silently follows into a bogus `404 "Resource not found"` — the real 422 and its field errors never reach the test. This already produced two wrong entries in `PRODUCT_EXPLORATION.md` before it was found. Consequence for new request objects: take `headers` as a **plain** constructor parameter, never a `protected` parameter property — a parameter property is assigned after `super()` and would overwrite the base's merged headers with the raw argument.
- **Catalog writes are negative-only, and target `UNKNOWN_ID` (`src/api/data/catalog.data.ts`) — never a live row.** The API resolves existence _before_ auth on `PUT`/`PATCH` and validates _before_ auth on `POST`, so pointing a write probe at a real product/brand/category risks mutating shared production data to answer a question about auth. Whether a _valid_ anonymous `POST` creates a catalog row is untested by design — see `PRODUCT_EXPLORATION.md` §4 before extending `tests/api/catalog/`.
- **Every user the API registers is permanent, and passwords are never hard-coded.** A customer cannot delete their own account (`DELETE /users/{ownId}` → 403; deletion is admin-only and out of scope), so each `registerUserWithApi()` call writes a row to the shared production DB that nothing can clean up — don't pull a `*RequestLogged` fixture into a spec that doesn't need auth. Separately, both register and change-password reject passwords found in a breach corpus ("The given password has appeared in a data leak"), so a literal password is a time bomb that eventually fails every test using it, with an error naming the password rather than the rule under test — always use `prepareRandomPassword()`.
- **Specs** live under `tests/ui/` (with `tests/ui/smoke/`) and `tests/api/<area>/<name>.api.spec.ts`. API specs run under the browserless `api` project in `playwright.config.ts` (`npx playwright test --project=api`); the browser projects ignore `tests/api/**`.
- **Auth via `storageState`** (`playwright.config.ts` project graph): the `setup` project (`tests/setup/login.setup.ts`) registers a fresh user via the API factory, logs in through the UI, and saves the session to `tmp/session.json` plus the user's credentials to `tmp/session-user.json` (paths in `config/storage.config.ts`, both gitignored). The `chromium-logged` project (`dependencies: ['setup']`, `grep: /@logged/`, `storageState`) runs `@logged`-tagged specs already authenticated; the `chromium` project (`grepInvert: /@logged/`) runs everything else. Because the API's JWTs expire after 5 minutes, the saved token alone would go stale mid-run — `src/ui/fixtures/logged-session.fixture.ts` (merged into `@src/fixtures/merge.fixture`) re-logs-in via the API as the setup-registered user before each `@logged` test and injects the fresh token as in-memory `storageState`. Tag a spec `@logged` to get this session instead of logging in inline.
