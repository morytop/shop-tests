...
| Favorites — own data                   | `favorites/favorites.crud.api.spec.ts`           | Add/list/read/remove round-trip, duplicate 409, unknown-product 422, anonymous 401, unknown-delete 204 gap                                                                                                   | ✅     |
| Invoices — own orders                  | `invoices/invoices.crud.api.spec.ts`             | Cart→invoice→list/detail/search, guest order (+ missing-field 422), billing city/country cross-check, anonymous 401, empty-payload 422, unknown-cart 404, empty-cart $0 order, PDF status polling + download | ✅     |
...
| Contract — authenticated endpoints     | `contract/contract.logged.api.spec.ts`           | Register/login, `/users/me`, cart lifecycle, favorites, invoices (list + by-id) and contact-form bodies validated against the generated schemas — consolidated to register exactly two throwaway users       | ✅     |
