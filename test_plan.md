...
**Regression note (observed 2026-07-06):** `product-overview.spec.ts` › "last page's next pagination control is disabled" now fails against production. On the current catalog the last page's `[data-test="pagination-next"]` parent `<li>` stays `class="page-item"` (never gains `disabled`), so the `toHaveClass(/disabled/)` assertion times out. Verified against the original pre-`ProductListPage` code (identical failure), so this is an **app/data behavior change, not a regression from the §5.2 refactor**. The `isOnLastPage()` "next is disabled on the last page" assumption needs revisiting (the app may now remove the control, keep it enabled as a no-op, or paginate differently) — tracked as a §5.1 follow-up, out of scope for §5.2.
